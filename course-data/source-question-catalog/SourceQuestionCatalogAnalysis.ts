...
export type SourceQuestionFrequencyRow = {
  id: string;
  count: number;
  percentage: number;
};

function buildFrequencyRows(values: string[]): SourceQuestionFrequencyRow[] {
...
export function getSourceQuestionsForFamily(
  catalog: SourceQuestionCatalogEntry[],
  familyId: string
): SourceQuestionCatalogEntry[] {
  return catalog.filter((question) => question.familyId === familyId);
}

export function analyseSourceQuestionFamily(
  catalog: SourceQuestionCatalogEntry[],
  familyId: string
) {
  const questions = getSourceQuestionsForFamily(catalog, familyId);
