...
export type CourseCatalogEntry = {
  ...
  label: string;
  shortLabel: string;
  classCourseLabel: CourseClassCourseLabel;
  isAvailable: boolean;
};
...
export const COURSE_CATALOG: CourseCatalogEntry[] = [
  {
    ...
    isAvailable: SUPPORTED_COURSE_IDS.includes("N5_MATH"),
  },
  {
    id: "N5_APPLICATIONS_MATH",
    label: "National 5 Applications of Maths",
    shortLabel: "N5 Applications",
    classCourseLabel: "National 5 Applications",
    isAvailable: SUPPORTED_COURSE_IDS.includes("N5_APPLICATIONS_MATH"),
  },
  {
    id: "HIGHER_MATH",
    label: "Higher Maths",
    shortLabel: "Higher Maths",
    classCourseLabel: "Higher Maths",
    isAvailable: SUPPORTED_COURSE_IDS.includes("HIGHER_MATH"),
  },
];

export function normaliseCourseId(value: unknown): CourseId | null {
...
export function getCourseCatalogEntry(
  ...
  const courseId = normaliseCourseId(value);
  if (!courseId) return null;

  return COURSE_CATALOG.find((entry) => entry.id === courseId) ?? null;
}
...
export function getCourseCatalogEntryByClassCourseLabel(
  ...
  return (
    COURSE_CATALOG.find((entry) => entry.classCourseLabel === value) ?? null
  );
}
...
export function isCourseAvailable(value: unknown): boolean {
  return getCourseCatalogEntry(value)?.isAvailable ?? false;
}
...
export function getDefaultClassCourseLabel(): CourseClassCourseLabel {
  return (
    getCourseCatalogEntry(getDefaultCourseId())?.classCourseLabel ??
    "National 5 Maths"
  );
}