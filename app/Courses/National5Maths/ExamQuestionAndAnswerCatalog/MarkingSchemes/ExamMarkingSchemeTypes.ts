...
import type {                                                                                      /* Opens shared catalogue imports from the Question contract. */
  ...
  ExamCatalogueReviewStatus,                                                                      /* Reuses the shared review stages. */
  ExamCatalogueValue,                                                                             /* Reuses explicit VALUE and non-value states. */
...
} from "../Questions/ExamQuestionTypes";                                                         /* Closes shared catalogue imports. */
...
export type ExamMarkingSchemeSourceKind =                                                        /* Describes the type of source marking document. */
  | "STANDARD_EXAM"                                                                               /* A normal external assessment MS. */
  | "MODIFIED_EXAM"                                                                               /* An MS for a modified assessment. */
  | "ASSESSMENT_RESOURCE"                                                                         /* An MS for an assessment resource. */
  | "SPECIMEN"                                                                                    /* A specimen or practice MS. */
  | "TEACHER_AUTHORED"                                                                            /* A teacher-created MS. */
  | "OTHER";                                                                                      /* Any source that does not fit the named groups. */

export type ExamMarkingSchemeSourceContext = {                                                   /* Opens the source context for this MS. */
  sourceKind: ExamMarkingSchemeSourceKind;                                                       /* Records the assessment regime. */
  ...
  generalMarkingPolicyId: string;                                                                 /* Links to the matching year or regime policy. */
...
export type ExamGeneralMarkingPolicySnapshot = {                                                 /* Opens the policy details relevant to this MS. */
  policyId: string;                                                                               /* Gives the general policy a stable ID. */
  policyVersion: string;                                                                          /* Records the year or regime version. */
  positiveMarking: boolean;                                                                       /* Records whether positive marking applies. */
  oneMarkPerNode: boolean;                                                                        /* Records whether every mark node is worth one mark. */
  transcriptionErrorRulePresent: boolean;                                                        /* Records whether transcription-error guidance applies. */
  horizontalVerticalRulePresent: boolean;                                                        /* Records whether horizontal or vertical marking can apply. */
  repeatedErrorRulePresent: boolean;                                                             /* Records whether repeated-error guidance applies. */
  showThatRulePresent: boolean;                                                                  /* Records whether stated-result follow-through guidance applies. */
  multipleAttemptRulePresent: boolean;                                                           /* Records whether multiple-attempt guidance applies. */
  sourceEvidence: ExamCatalogueEvidenceRef[];                                                     /* Links the policy snapshot to the source guidance. */
};                                                                                                /* Closes the general-policy snapshot. */
...
export type ExamMarkingSchemeReviewProfile = {                                                   /* Opens the review record for this MS entry. */
...
// ============================================================================
// SECTION 19 — COMPLETE MARKING SCHEME CATALOGUE ENTRY
// ============================================================================
...
export type ExamMarkingSchemeCatalogEntry = {                                                    /* Opens the universal Marking Scheme Catalogue entry. */
  ...
  generalPolicy: ExamGeneralMarkingPolicySnapshot;                                                /* Stores the relevant general-policy snapshot. */
  ...
  review: ExamMarkingSchemeReviewProfile;                                                         /* Stores review state and unresolved issues. */
