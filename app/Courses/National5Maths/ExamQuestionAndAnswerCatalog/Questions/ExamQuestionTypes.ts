...
// ============================================================================
// SECTION 1 — SHARED CATALOGUE FOUNDATIONS
// ============================================================================
...
export type ExamPaperContextId = string;                                                         /* Links a Question to its paper context. */
...
export type ExamCatalogueEvidenceId = string;                                                    /* Gives each saved source reference a stable ID. */

export type ExamCatalogueValueState =                                                            /* Defines the state of a catalogue field. */
  | "VALUE"                                                                                      /* A reviewed value is known. */
  | "NOT_APPLICABLE"                                                                             /* The field genuinely does not apply. */
  | "UNKNOWN"                                                                                    /* The field applies but cannot be confirmed. */
...
export type ExamCatalogueProvenanceKind =                                                        /* Says where a catalogue value comes from. */
  | "SOURCE_FACT"                                                                                /* The source document directly shows it. */
  | "CATALOGUE_CLASSIFICATION"                                                                   /* We classified it from the source evidence. */
  | "GENERATION_ANALYSIS";                                                                       /* We derived it to support future generation. */

export type ExamCatalogueEvidenceType =                                                          /* Says which source supplied the evidence. */
  | "QUESTION"                                                                                   /* Evidence comes from the Question Paper. */
  | "MARKING_SCHEME"                                                                             /* Evidence comes from the Marking Scheme. */
  | "GENERAL_MARKING_POLICY"                                                                     /* Evidence comes from general marking guidance. */
  | "VISUAL";                                                                                    /* Evidence comes from a rendered visual. */

export type ExamCatalogueEvidenceRef = {                                                         /* Opens a traceable source reference. */
  id: ExamCatalogueEvidenceId;                                                                   /* Gives this source reference a stable ID. */
  ...
  questionLocator: string | null;                                                                /* Stores the Question or part label if relevant. */
  notes: string | null;                                                                          /* Adds a short source-location note when needed. */
};                                                                                               /* Closes the source reference. */

export type ExamCatalogueProvenance = {                                                          /* Opens provenance for an analysed value. */
  kind: ExamCatalogueProvenanceKind;                                                             /* Records whether the value is source, catalogue, or generator knowledge. */
  evidence: ExamCatalogueEvidenceRef[];                                                         /* Links the value back to supporting evidence. */
};                                                                                               /* Closes provenance. */

export type ExamCatalogueKnownValue<T> = {                                                       /* Opens a catalogue field with a known value. */
  state: "VALUE";                                                                                /* Confirms that the field has been reviewed and valued. */
  value: T;                                                                                      /* Stores the reviewed value. */
  reason: null;                                                                                  /* Keeps the reason empty for an ordinary known value. */
  confidence: ExamCatalogueConfidence | null;                                                    /* Stores confidence when judgement was needed. */
  provenance: ExamCatalogueProvenance;                                                           /* Records where the value came from. */
};                                                                                               /* Closes the known-value form. */

export type ExamCatalogueNotApplicableValue = {                                                  /* Opens a field that does not apply. */
  state: "NOT_APPLICABLE";                                                                       /* Confirms that the field is genuinely irrelevant here. */
  value: null;                                                                                   /* Stores no value because the field does not apply. */
  reason: string | null;                                                                         /* Optionally explains why it does not apply. */
  confidence: null;                                                                              /* Needs no confidence score. */
  provenance: ExamCatalogueProvenance;                                                           /* Records who made the applicability decision. */
};                                                                                               /* Closes the not-applicable form. */

export type ExamCatalogueUnknownValue = {                                                        /* Opens a field that cannot be confirmed. */
  state: "UNKNOWN";                                                                              /* Confirms that the field matters but cannot be determined. */
  value: null;                                                                                   /* Stores no guessed value. */
  reason: string;                                                                                /* Explains why the value cannot be confirmed. */
  confidence: null;                                                                              /* Needs no confidence score because no value was chosen. */
  provenance: ExamCatalogueProvenance;                                                           /* Records the evidence checked. */
};                                                                                               /* Closes the unknown form. */

export type ExamCatalogueNotReviewedValue = {                                                    /* Opens a temporary unreviewed field. */
  state: "NOT_REVIEWED";                                                                         /* Marks the field as still awaiting review. */
  value: null;                                                                                   /* Stores no value before review. */
  reason: null;                                                                                  /* Keeps the reason empty until review happens. */
  confidence: null;                                                                              /* Keeps confidence empty until review happens. */
  provenance: ExamCatalogueProvenance;                                                           /* Records the current catalogue source context. */
};                                                                                               /* Closes the unreviewed form. */

export type ExamCatalogueValue<T> =                                                              /* Wraps any catalogue field that may not have a value. */
  | ExamCatalogueKnownValue<T>                                                                   /* Allows a reviewed known value. */
  | ExamCatalogueNotApplicableValue                                                              /* Allows a genuine not-applicable state. */
  | ExamCatalogueUnknownValue                                                                    /* Allows an evidence-limited unknown state. */
  | ExamCatalogueNotReviewedValue;                                                               /* Allows a temporary unreviewed state. */

export type ExamCatalogueReviewStatus =                                                          /* Records the review stage of a whole entry. */
  | "DRAFT"                                                                                      /* The entry is still being built. */
  | "CATALOGUED"                                                                                 /* The first catalogue pass is complete. */
  | "REVIEWED"                                                                                   /* A second review has been completed. */
  | "APPROVED"                                                                                   /* The entry is approved for normal use. */
  | "DEPRECATED";                                                                                /* The entry remains for history but should not be used. */

// ============================================================================
// SECTION 2 — SOURCE AND PAPER CONTEXT
// ============================================================================

export type ExamQuestionSourceKind =                                                             /* Describes the type of historical source. */
  | "STANDARD_EXAM"                                                                              /* A normal external assessment paper. */
  | "MODIFIED_EXAM"                                                                              /* A formally modified assessment paper. */
  | "ASSESSMENT_RESOURCE"                                                                        /* An assessment resource rather than a normal exam. */
  | "SPECIMEN"                                                                                   /* A specimen or practice source. */
  | "TEACHER_AUTHORED"                                                                           /* A teacher-created source. */
  | "OTHER";                                                                                     /* Any source that does not fit the named groups. */

export type ExamQuestionCalculatorStatus =                                                       /* Records calculator use for the Question. */
  | "NON_CALCULATOR"                                                                             /* A calculator must not be used. */
  | "CALCULATOR_ALLOWED"                                                                         /* A calculator may be used. */
  | "CALCULATOR_REQUIRED";                                                                       /* The intended task relies on calculator use. */

export type ExamQuestionPaperContext = {                                                         /* Opens the paper-level context saved with a Question. */
  paperContextId: ExamPaperContextId;                                                            /* Gives this paper context a stable ID. */
  sourceKind: ExamQuestionSourceKind;                                                            /* Records the assessment regime. */
  year: number;                                                                                  /* Records the source year. */
  paper: Paper;                                                                                  /* Records the source paper. */
  calculatorStatus: ExamQuestionCalculatorStatus;                                                /* Records the calculator rule. */
  durationMinutes: number;                                                                       /* Records the paper duration. */
  paperTotalMarks: number;                                                                       /* Records the total marks available on the paper. */
  numberedQuestionCount: number;                                                                 /* Records the number of numbered Questions on the paper. */
  formulaSheetProvided: boolean;                                                                 /* Records whether a formula list was supplied. */
  formulaSheetProfileId: string | null;                                                          /* Links to a known formula-sheet profile when available. */
  generalMarkingPolicyId: string;                                                                /* Links to the matching general marking policy. */
  sourceEvidence: ExamCatalogueEvidenceRef[];                                                    /* Links back to the paper-level source evidence. */
};                                                                                               /* Closes the paper context. */

// ============================================================================
// SECTION 3 — CATALOGUE IDENTITY
// ============================================================================
...
// ============================================================================
// SECTION 4 — SOURCE LOCATION AND PAGE LAYOUT
// ============================================================================
...
// ============================================================================
// SECTION 9 — INFORMATION AND REASONING DEMAND
// ============================================================================
...
// ============================================================================
// SECTION 10 — NUMBER AND PARAMETER DESIGN
// ============================================================================
...
// ============================================================================
// SECTION 11 — ANSWER SPECIFICATION
// ============================================================================
...
// ============================================================================
// SECTION 12 — CONTEXT AND LANGUAGE
// ============================================================================
...
export type ExamQuestionMathematicalModelProfile = {                                             /* Opens a contextual mathematical-model description. */
  ...
  amplitudePresent: boolean;                                                                      /* Records whether an amplitude term is present. */
  verticalShiftPresent: boolean;                                                                  /* Records whether a vertical shift is present. */
  phaseShiftPresent: boolean;                                                                     /* Records whether a phase shift is present. */
};                                                                                               /* Closes the mathematical-model profile. */

// ============================================================================
// SECTION 15 — SPECIALISED MATHEMATICAL PROFILES
// ============================================================================
...
// ============================================================================
// SECTION 18 — REVIEW AND NOTES
// ============================================================================

export type ExamQuestionReviewProfile = {                                                        /* Opens the review record for the Question entry. */
  ...
  catalogueNotes: string[];                                                                      /* Stores short internal notes that do not belong elsewhere. */
};                                                                                               /* Closes the review profile. */
...
export type ExamQuestionCatalogEntry = {                                                         /* Opens the universal Question Catalogue entry. */
  ...
  paperContext: ExamQuestionPaperContext;                                                        /* Stores the source paper context. */
  ...
  review: ExamQuestionReviewProfile;                                                             /* Stores review state and unresolved issues. */
