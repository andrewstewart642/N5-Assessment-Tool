...
export const N2_GENERAL_SURFACE_GUARDRAILS = [
  ...
  "Use one direct mathematical command, with a second short output-form instruction only where the positive-power requirement is part of the selected mechanism.",
  "Do not add narrative context merely to create surface variety; that would introduce an unsupported reasoning/context family.",
  "Use one algebraic base per same-base law chain. Change the symbol freely, but keep the expression visually compact.",
  "Preserve clear fraction bars and exponent typesetting; visual ambiguity in powers or denominators is not an acceptable variety mechanism.",
  "Avoid reproducing any historical expression exactly or by trivial symbol substitution.",
...
export const N2_FRACTIONAL_SURFACE_GUARDRAILS = [
  ...
  "Choose the base so the relevant root is exact and recognisable without a calculator.",
  "Keep the surface to a single displayed power; extra products, sums or nested powers move the item into another family.",
  "Vary the base and exponent within the calibrated exact-integer envelope rather than changing the prompt grammar.",
] as const;
...
export const N2_BRACKETED_SURFACE_GUARDRAILS = [
  ...
  "For distributive mode, at most one term should collapse to a zero power/constant unless later evidence supports a different two-mark structure.",
] as const;
