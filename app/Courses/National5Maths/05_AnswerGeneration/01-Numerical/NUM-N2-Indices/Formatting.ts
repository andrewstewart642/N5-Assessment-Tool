...
import type { N2RationalExponent } from "../../../04_QuestionGeneration/01-Numerical/NUM-N2-Indices/Types";
...
export const powerPlain = (variable: string, exponent: number | N2RationalExponent) => {
  const body = typeof exponent === "number" ? `${exponent}` : rationalExponentPlain(exponent);
  return `${variable}^(${body})`;
};

export const powerLatex = (variable: string, exponent: number | N2RationalExponent) => {
  const body = typeof exponent === "number" ? `${exponent}` : rationalExponentLatex(exponent);
  return `${variable}^{${body}}`;
};

export const fractionLatex = (numerator: string, denominator: string) => `\\frac{${numerator}}{${denominator}}`;
