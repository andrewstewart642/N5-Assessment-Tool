...
const FAMILY_DIFFICULTIES: Record<G1GeneratorFamily, readonly G1GeneratorDifficulty[]> = {
  LINE_EQUATION_FROM_TWO_POINTS: [1],
  CONTEXTUAL_LINEAR_MODEL: [2],
  BEST_FIT_LINEAR_MODEL: [1, 2],
  SYMBOLIC_GRADIENT_FROM_TWO_POINTS: [2],
};

const FAMILY_WEIGHTS: Record<G1GeneratorFamily, number> = {
  LINE_EQUATION_FROM_TWO_POINTS: 6,
  CONTEXTUAL_LINEAR_MODEL: 3,
  BEST_FIT_LINEAR_MODEL: 2,
...
function Chip({ children, emphasis = false, warning = false }: { children: ReactNode; emphasis?: boolean; warning?: boolean }) {
  ...
  const color = warning ? "#fde68a" : emphasis ? "#a7f3d0" : "#cbd5e1";
  return (
    <span
      style={{
        display: "inline-flex",
...
function PolicySummary({ answer }: { answer: G1GeneratedMarkingScheme }) {
  const presentation = Object.entries(answer.presentationPolicy)
    .filter(([, value]) => Boolean(value))
    .map(([key]) => formatToken(key).toLowerCase());
  return (
    <div style={{ display: "grid", gap: 4, color: "#94a3b8", fontSize: 9, lineHeight: 1.4 }}>
      <div><strong style={{ color: "#cbd5e1" }}>Working:</strong> required; correct answer without working = no credit; equivalent line forms {answer.workingPolicy.equivalentLineFormsAccepted ? "accepted" : "not used"}.</div>
      <div><strong style={{ color: "#cbd5e1" }}>Presentation:</strong> {presentation.join("; ")}.</div>
      <div><strong style={{ color: "#cbd5e1" }}>Follow-through:</strong> {answer.workingPolicy.followThroughPrinciple}</div>
    </div>
  );
}
          ...
          {answer.markPoints.map((mark) => (
            ...
            >
              <div style={{ padding: 8, borderRight: "1px solid #e5e7eb", textAlign: "center", fontWeight: 800, fontSize: 10 }}>
                ...
                <div style={{ marginTop: 3, color: "#64748b", fontSize: 8 }}>{mark.standard} · {mark.thinking}</div>
              </div>
              <div style={{ padding: "8px 10px", fontSize: 10, lineHeight: 1.45 }}>
                ...
                {mark.evidenceExamples.length ? (
                  <div style={{ marginTop: 4, color: "#475569", fontSize: 9 }}>
                    Evidence: {mark.evidenceExamples.join(" | ")}
                  </div>
                ) : null}
                ...
                {mark.blockingConditions.length ? (
                  <div style={{ marginTop: 3, color: "#991b1b", fontSize: 9 }}>
                    Block: {mark.blockingConditions.join(" ")}
                  </div>
                ) : null}
              </div>
            </div>
        ...
        <div style={{ display: "grid", gap: 7 }}>
          {answer.methods.map((method) => (
            ...
            >
              ...
              <div className="g1-math-surface" style={{ display: "grid", gap: 8, padding: 10, background: "#ffffff", color: "#111827" }}>
...
function ValidationWarnings({ sample }: { sample: GeneratedG1Sample }) {
  const warnings = [
    ...(sample.questionIssues ?? []).filter((issue) => issue.severity === "WARNING"),
    ...(sample.answerIssues ?? []).filter((issue) => issue.severity === "WARNING"),
  ];
  if (!warnings.length) return null;
  return (
    <section style={{ padding: "7px 9px", border: "1px solid rgba(251,191,36,0.25)", borderRadius: 7, background: "rgba(245,158,11,0.07)", color: "#fde68a", fontSize: 9, lineHeight: 1.45 }}>
      {warnings.map((warning) => <div key={`${warning.code}-${warning.message}`}><strong>{warning.code}</strong>: {warning.message}</div>)}
    </section>
  );
}
    ...
    <article style={{ border: "1px solid rgba(148,163,184,0.18)", borderRadius: 10, overflow: "hidden", background: "rgba(255,255,255,0.03)" }}>
      ...
      <div style={{ display: "grid", gap: 12, padding: 10 }}>
        <section>
          ...
          <div
            className="g1-question-preview g1-math-surface"
            style={{
              padding: "16px 18px",
              borderRadius: 7,
              background: "#ffffff",
        ...
        <section>
          ...
          {difficulty.signalLabels.length ? (
            <div style={{ marginTop: 6, color: "#94a3b8", fontSize: 9, lineHeight: 1.4 }}>
              Difficulty signals: {difficulty.signalLabels.join("; ")}
            </div>
          ) : null}
          {question.family === "BEST_FIT_LINEAR_MODEL" ? (
            <div style={{ marginTop: 6, color: "#fde68a", fontSize: 9, lineHeight: 1.4 }}>
              Composite boundary: this generated item deliberately stops at {question.deferredComposite.g1MarksGenerated} G1 marks. The adjacent {question.deferredComposite.embeddedMarksDeferred}-mark statistical component remains deferred.
            </div>
          ) : null}
        </section>

        <ValidationWarnings sample={sample} />

        <section>
          <SectionLabel>Final answer</SectionLabel>
          <div className="g1-final-answer g1-math-surface" style={{ padding: "9px 11px", borderRadius: 7, background: "rgba(255,255,255,0.055)", color: "#f8fafc", lineHeight: 1.7 }}>
            {answer.finalAnswers.map((finalAnswer, answerIndex) => (
              <div key={`${finalAnswer.partLabel}-${answerIndex}`} style={{ marginBottom: answerIndex < answer.finalAnswers.length - 1 ? 8 : 0 }}>
                {finalAnswer.partLabel ? <strong style={{ marginRight: 6, color: "#cbd5e1", fontSize: 10 }}>({finalAnswer.partLabel})</strong> : null}
                <PaperContent parts={asMathParts(finalAnswer.latex)} />
                <div style={{ marginTop: 3, color: "#94a3b8", fontSize: 8.5 }}>
                  {finalAnswer.normalisedAnswer}{finalAnswer.unit ? ` ${finalAnswer.unit}` : ""}
                </div>
              </div>
            ))}
          </div>
        ...
        <details style={{ borderTop: "1px solid rgba(148,163,184,0.10)", paddingTop: 5 }}>
          ...
          <div style={{ marginTop: 6, color: "#94a3b8", fontSize: 8.5, lineHeight: 1.45 }}>
            ...
            Calibration anchors: {question.quality.calibrationSourceAnchorIds.join(", ")}<br />
            Answer profile anchors: {answer.profileSourceAnchorIds.join(", ")}
          </div>
          <pre style={{ marginTop: 6, maxHeight: 440, overflow: "auto", padding: 8, borderRadius: 7, background: "rgba(0,0,0,0.28)", color: "#94a3b8", fontSize: 8, lineHeight: 1.35, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>
...
export default function G1GeneratorTesterPage() {
  ...
  const [baseSeed, setBaseSeed] = useState<number>(61001);
  ...
  const selectedFamilyProfile = useMemo(
    () => selectedFamily === "MIX" ? null : getG1FamilyProfile(selectedFamily),
    [selectedFamily],
  );

  const supportedPapers = useMemo<readonly G1GeneratorPaper[]>(() => {
    if (!selectedFamilyProfile) return PAPERS;
    return selectedFamilyProfile.supportedPapers as readonly G1GeneratorPaper[];
  }, [selectedFamilyProfile]);
  const effectivePaper = supportedPapers.includes(paper) ? paper : supportedPapers[0] ?? "P1";

  const surfaceOptions = useMemo<readonly G1GeneratorSurfaceStyle[]>(
    () => selectedFamily === "MIX" ? [] : G1_SURFACES_BY_FAMILY[selectedFamily],
    [selectedFamily],
  );

  const difficultyOptions = useMemo<readonly G1GeneratorDifficulty[]>(
    () => selectedFamily === "MIX" ? [1, 2] : FAMILY_DIFFICULTIES[selectedFamily],
    [selectedFamily],
  );
  ...
  const candidateFamilies = useMemo(() => {
    ...
    return FAMILIES.filter((family) => {
      ...
      if (!includeExperimental && profile.readiness === "EXPERIMENTAL") return false;
      if (!includeDeferred && profile.readiness === "COMPOSITE_DEFERRED") return false;
      if (difficulty !== "AUTO" && !FAMILY_DIFFICULTIES[family].includes(difficulty)) return false;
      return true;
    });
  }, [difficulty, effectivePaper, includeDeferred, includeExperimental, selectedFamily]);

  function familyForSeed(seed: number): G1GeneratorFamily {
    if (selectedFamily !== "MIX") return selectedFamily;
    const weighted = candidateFamilies.flatMap((family) => Array.from({ length: FAMILY_WEIGHTS[family] }, () => family));
    if (!weighted.length) throw new Error("No G1 family is enabled for the selected paper/difficulty/readiness controls.");
    const slot = Math.abs((seed * 37 + 11) % weighted.length);
    return weighted[slot];
  }
    ...
    return Array.from({ length: sampleCount }, (_, index): GeneratedG1Sample => {
      ...
      try {
        ...
        const question = generateG1Question({
          ...
          includeExperimentalFamilies: selectedFamily === "MIX" ? includeExperimental : true,
          includeDeferredCompositeFamilies: selectedFamily === "MIX" ? includeDeferred : true,
        });
        ...
        return {
          ...
          question,
          answer,
          questionIssues: questionValidation.issues,
          answerIssues: answerValidation.issues,
        };
      } catch (error) {
        return {
          id: `error-${seed}-${index}`,
  ...
  useEffect(() => {
    const startSeed = 61001;
    setSamples(buildSamples(startSeed));
    setLastBatchStartSeed(startSeed);
    setBaseSeed(startSeed + 10);
    // Initial generation only; subsequent batches deliberately use current controls and fresh seeds.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  const failedCount = samples.filter((sample) => Boolean(sample.error)).length;
  const successfulSamples = samples.filter(
    (sample): sample is GeneratedG1Sample & { question: G1GeneratedQuestion; answer: G1GeneratedMarkingScheme } =>
  ...
  return (
    <main style={{ minHeight: "100vh", padding: 16, background: "#070a10", color: "#f8fafc", fontFamily: "Inter, ui-sans-serif, system-ui, sans-serif" }}>
      <style>{`
        .g1-math-surface .katex { font-size: 1.12em; }
        .g1-question-preview .katex { line-height: 1.45; }
        .g1-final-answer .katex { font-size: 1.2em; }
      `}</style>

      <div style={{ width: "min(1680px, 100%)", margin: "0 auto" }}>
        <div style={{ marginBottom: 10 }}>
          <div style={{ color: "#e2e8f0", fontSize: 15, fontWeight: 850 }}>G1 · Gradient and equation of a straight line</div>
          <div style={{ marginTop: 3, color: "#64748b", fontSize: 10 }}>
            Canonical G1 question + answer stress tester. Visuals below are developer diagnostics generated from the question visual specification, not production layer-06 artwork.
          </div>
        </div>
        ...
        <section style={{ display: "flex", alignItems: "end", gap: 8, flexWrap: "wrap", padding: 10, marginBottom: 8, border: "1px solid rgba(148,163,184,0.18)", borderRadius: 10, background: "rgba(255,255,255,0.03)" }}>
          <ControlLabel>
            ...
            <select
              ...
              onChange={(event) => {
                const next = event.target.value as SelectedFamily;
                setSelectedFamily(next);
                setSelectedSurface("AUTO");
                setDifficulty("AUTO");
                setControlsDirty(true);
              }}
              style={{ ...controlStyle, minWidth: 285 }}
            >
              <option value="MIX">Calibrated mixed bag</option>
              {FAMILIES.map((family) => <option key={family} value={family}>{FAMILY_LABELS[family]}</option>)}
            </select>
          </ControlLabel>
          ...
          <ControlLabel>
            ...
            <select
              ...
              disabled={selectedFamily === "MIX"}
              onChange={(event) => {
                setSelectedSurface(event.target.value as SelectedSurface);
                setDifficulty("AUTO");
                setControlsDirty(true);
              }}
              style={{ ...controlStyle, minWidth: 265, opacity: selectedFamily === "MIX" ? 0.55 : 1 }}
            >
              <option value="AUTO">Auto surface</option>
              {surfaceOptions.map((surface) => <option key={surface} value={surface}>{SURFACE_LABELS[surface]}</option>)}
            </select>
          </ControlLabel>
          ...
          <ControlLabel>
            ...
            <select
              value={difficulty}
              onChange={(event) => {
                setDifficulty(event.target.value === "AUTO" ? "AUTO" : Number(event.target.value) as G1GeneratorDifficulty);
                setControlsDirty(true);
              }}
              style={{ ...controlStyle, minWidth: 145 }}
            >
              <option value="AUTO">Auto calibrated</option>
              {difficultyOptions.map((value) => <option key={value} value={value}>{value === 1 ? "Lower band (L1)" : "Upper band (L2)"}</option>)}
            </select>
          </ControlLabel>
          ...
          <ControlLabel>
            ...
            <select
              ...
              onChange={(event) => {
                setPaper(event.target.value as G1GeneratorPaper);
                setControlsDirty(true);
              }}
              style={controlStyle}
            >
          ...
          <ControlLabel>
            Next batch seed
            <input type="number" value={baseSeed} onChange={(event) => setBaseSeed(Number(event.target.value) || 1)} style={{ ...controlStyle, width: 112 }} />
          </ControlLabel>
          ...
          <button type="button" onClick={() => setShowVisualOverlay((current) => !current)} style={{ height: 32, padding: "0 13px", border: "1px solid rgba(148,163,184,0.28)", borderRadius: 7, background: showVisualOverlay ? "rgba(59,130,246,0.15)" : "rgba(255,255,255,0.04)", color: "#bfdbfe", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>
            {showVisualOverlay ? "Hide graph diagnostics" : "Show graph diagnostics"}
          </button>
        </section>

        {selectedFamily === "MIX" ? (
          <section style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center", marginBottom: 8, padding: "7px 10px", border: "1px solid rgba(148,163,184,0.12)", borderRadius: 8, color: "#94a3b8", fontSize: 9.5 }}>
            <label style={{ display: "flex", gap: 6, alignItems: "center", cursor: "pointer" }}>
              <input type="checkbox" checked={includeDeferred} onChange={(event) => { setIncludeDeferred(event.target.checked); setControlsDirty(true); }} />
              Include deferred best-fit G1 shell
            </label>
            <label style={{ display: "flex", gap: 6, alignItems: "center", cursor: "pointer" }}>
              <input type="checkbox" checked={includeExperimental} onChange={(event) => { setIncludeExperimental(event.target.checked); setControlsDirty(true); }} />
              Include experimental symbolic family
            </label>
            <span>Enabled families for {effectivePaper}{difficulty === "AUTO" ? "" : ` L${difficulty}`}: {candidateFamilies.map((family) => FAMILY_LABELS[family]).join(" · ") || "none"}</span>
          </section>
        ) : null}

        <section style={{ display: "flex", gap: 5, flexWrap: "wrap", alignItems: "center", marginBottom: 8 }}>
          ...
          {selectedFamilyProfile ? <Chip>{formatToken(selectedFamilyProfile.readiness)}</Chip> : null}
        </section>

        <section style={{ marginBottom: 10, padding: "7px 10px", border: "1px solid rgba(148,163,184,0.12)", borderRadius: 8, color: "#94a3b8", fontSize: 9.5, lineHeight: 1.5 }}>
          <div><strong style={{ color: "#cbd5e1" }}>Batch families:</strong> {Object.entries(familyCounts).map(([family, count]) => `${formatToken(family)} · ${count}`).join("   |   ") || "none"}</div>
          <div><strong style={{ color: "#cbd5e1" }}>Batch surfaces:</strong> {Object.entries(surfaceCounts).map(([surface, count]) => `${formatToken(surface)} · ${count}`).join("   |   ") || "none"}</div>
        </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0, 1fr))", gap: 10 }}>
          {samples.map((sample, index) => (
            <SampleCard
              ...
              showVisualOverlay={showVisualOverlay}
            />
          ))}
        </div>
      </div>
