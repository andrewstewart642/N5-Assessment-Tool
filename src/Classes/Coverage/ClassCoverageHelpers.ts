...
export function getCoverageCategoryEntries(
  courseId: CourseId
): Array<[string, SkillLike[]]> {
  const courseConfig =
    getCourseAssessmentConfigById(courseId);
...
export function getCoverageSkillById(
  ...
  return (
    getAllCoverageSkills(courseId).find(
      (entry) =>
        entry.skill.id === skillId
    ) ?? null
  );
}
